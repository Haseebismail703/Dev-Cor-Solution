      ...
      <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:300,background:"rgba(6,11,20,.92)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,255,255,.07)"}}>
        <div style={{maxWidth:1200,margin:"0 auto",padding:"0 24px",display:"flex",alignItems:"center",justifyContent:"space-between",height:66}}>
          <div style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}} onClick={()=>scrollTo("home")}>
            <LogoSVG size={36}/>
            <div>
              <div style={{fontWeight:800,fontSize:16,lineHeight:1.1}}><span style={{color:"#fff"}}>Dev</span><span style={{color:"#38bdf8"}}>Core</span></div>
          ...
          {!isMobile&&(
            <div style={{display:"flex",alignItems:"center",gap:4}}>
              ...
              <a href="mailto:devcorsolution@gmail.com" style={{color:"rgba(255,255,255,.6)",fontSize:13,textDecoration:"none",marginRight:8}}>devcorsolution@gmail.com</a>
              <button onClick={()=>scrollTo("contact")} style={{background:"#2563eb",border:"none",color:"#fff",padding:"9px 20px",borderRadius:7,fontWeight:700,fontSize:14,cursor:"pointer"}}>Get In Touch</button>
            </div>
          )}
          {isMobile&&<button onClick={()=>setMenuOpen(o=>!o)} style={{background:"none",border:"none",color:"#fff",fontSize:22,cursor:"pointer"}}>{menuOpen?"✕":"☰"}</button>}
        </div>
        {isMobile&&menuOpen&&(
          <div style={{background:"#060b14",borderTop:"1px solid rgba(255,255,255,.07)",padding:"12px 24px 20px"}}>
            {NAV_LINKS.map(n=>(
              <button key={n} onClick={()=>scrollTo(n.toLowerCase())} style={{display:"block",width:"100%",background:"none",border:"none",color:"rgba(255,255,255,.7)",padding:"13px 0",fontSize:16,cursor:"pointer",textAlign:"left",borderBottom:"1px solid rgba(255,255,255,.06)"}}>{n}</button>
            ))}
            <button onClick={()=>scrollTo("contact")} style={{marginTop:14,width:"100%",background:"#2563eb",border:"none",color:"#fff",padding:"13px",borderRadius:8,fontWeight:700,fontSize:15,cursor:"pointer"}}>Get In Touch</button>
          </div>
        )}
      </nav>

      {/* HERO */}
      <section id="home" style={{minHeight:"100vh",display:"flex",alignItems:"center",padding:"120px 24px 80px",position:"relative",overflow:"hidden",background:"linear-gradient(160deg,#060b14 0%,#0a1628 60%,#060b14 100%)"}}>
        <div style={{position:"absolute",top:"10%",left:"50%",transform:"translateX(-50%)",width:"80vw",height:"60vh",background:"radial-gradient(ellipse,rgba(37,99,235,.12) 0%,transparent 65%)",pointerEvents:"none"}}/>
        <div style={{position:"absolute",inset:0,backgroundImage:"radial-gradient(rgba(255,255,255,.03) 1px,transparent 1px)",backgroundSize:"44px 44px",pointerEvents:"none"}}/>
        <div style={{maxWidth:1200,margin:"0 auto",width:"100%",textAlign:"center",animation:"fadeUp .8s ease both"}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(37,99,235,.1)",border:"1px solid rgba(37,99,235,.25)",borderRadius:24,padding:"7px 18px",marginBottom:28}}>
            ...
            <span style={{fontSize:12,color:"#86efac",fontWeight:600}}>Currently Accepting New Projects</span>
          </div>
          <h1 style={{fontSize:isMobile?32:60,fontWeight:900,lineHeight:1.1,marginBottom:20,letterSpacing:"-1px",color:"#fff"}}>
            Digital Transformation<br/>
            <span style={{background:"linear-gradient(135deg,#3b82f6,#38bdf8,#6366f1)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Company</span>
          </h1>
          <p style={{fontSize:isMobile?15:18,color:"rgba(255,255,255,.55)",maxWidth:600,margin:"0 auto 40px",lineHeight:1.8}}>
